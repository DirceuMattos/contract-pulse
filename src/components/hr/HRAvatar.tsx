...
const SIZE_CLASSES: Record<NonNullable<HRAvatarProps["size"]>, { box: string; text: string }> = {
...
export function HRAvatar({ nome, email, fotoUrl, size = "md", className }: HRAvatarProps) {
  const sz = SIZE_CLASSES[size];
  const initials = getInitials(nome);
  const colorClass = pickColor(nome);

  const gravatarUrl =
    !fotoUrl && email && email.trim()
      ? `https://www.gravatar.com/avatar/${md5(email.trim().toLowerCase())}?s=200&d=404`
      : null;

  const initialSrc = fotoUrl || gravatarUrl || undefined;
  ...
  return (
    <Avatar className={cn(sz.box, className)}>
      {src && (
        <AvatarImage
          src={src}
          alt={nome}
      ...
      <AvatarFallback className={cn(colorClass, sz.text, "font-medium")}>
        {initials}
      </AvatarFallback>
    </Avatar>
  );
