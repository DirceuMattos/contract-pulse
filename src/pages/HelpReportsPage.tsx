  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <nav className="hidden lg:flex flex-col w-52 shrink-0 border-r border-border overflow-y-auto p-3 gap-0.5">
          {SECTIONS.map(s => (
            <a
              ...
              className={`text-xs px-3 py-2 rounded-md transition-colors ${active === s.id ? 'bg-primary/10 text-primary font-semibold' : 'text-muted-foreground hover:bg-muted hover:text-foreground'}`}
            >
              {s.label}
            </a>
          ))}
        ...
        {/* Content */}
        <main ref={mainRef} className="flex-1 overflow-y-auto p-6 max-w-3xl">
          ...
          <SectionBlock id="criar" title="1. Criar um Novo Relatório">
            ...
            <Callout type="tip">Se o contrato já tem relatórios anteriores, aproveite a opção de cópia — ela traz Objetivo, Glossário e outras seções fixas, economizando tempo.</Callout>
          </SectionBlock>

          <SectionBlock id="editar" title="2. Editar as Seções do Relatório">
            <p className="text-sm text-muted-foreground mb-4">A tela de edição tem duas colunas: à esquerda a lista de seções com indicadores de preenchimento; à direita o editor da seção selecionada.</p>
            <h3 className="font-semibold text-sm mb-2">Indicadores de preenchimento</h3>
            <DataTable
              headers={['Ícone', 'Significado']}
              rows={[
                ['⬜', 'Seção vazia — ainda não preenchida'],
                ['🟡', 'Preenchimento parcial — alguns campos obrigatórios faltando'],
                ['✅', 'Seção completa'],
              ]}
            />
            <h3 className="font-semibold text-sm mb-2 mt-4">Salvamento automático</h3>
            <p className="text-sm text-muted-foreground mb-3">Não existe botão "Salvar". Cada alteração é salva automaticamente após 800ms de inatividade. O indicador <em>"Salvando dados..."</em> aparece no cabeçalho enquanto há alterações pendentes.</p>
            <h3 className="font-semibold text-sm mb-2">Ocultar slide no PPTX</h3>
            <p className="text-sm text-muted-foreground mb-3">Cada seção tem o toggle <strong>"Ocultar slide na geração do PPT"</strong>. Ative para excluir aquela seção do arquivo final sem precisar deletá-la.</p>
            <h3 className="font-semibold text-sm mb-2">Copiar do mês anterior</h3>
            <p className="text-sm text-muted-foreground mb-3">Caso o relatório já exista, use o botão <strong>Copiar mês anterior</strong> no cabeçalho. O sistema copia todas as seções manuais com conteúdo do relatório mais recente do mesmo contrato.</p>
          ...
          <SectionBlock id="sincronizar" title="3. Sincronizar Dados das Integrações">
            <p className="text-sm text-muted-foreground mb-4">Clique em <strong>🔄 Sincronizar Dados</strong> para buscar as informações atualizadas. Um painel mostra o resultado de cada integração:</p>
            <DataTable
              headers={['Ícone', 'Significado']}
              rows={[
                ['✅ Verde',    'Sincronizado com sucesso'],
                ['❌ Vermelho', 'Falha — o motivo aparece abaixo do nome da integração'],
                ['⚠️ Amarelo',  'Ignorado — ex: nenhum projeto Asana configurado'],
              ]}
            />
            <h3 className="font-semibold text-sm mb-2 mt-4">Integrações disponíveis</h3>
            <ul className="space-y-1 mb-4 list-none">
              {[
                ['📋 Asana', 'Importa entregas concluídas no mês e tarefas priorizadas do projeto configurado.'],
                ['🎙️ Fireflies', 'Importa reuniões e transcrições do período filtradas por domínio ou palavras-chave.'],
                ['🎫 Milvus', 'Importa tickets e demonstrativo de horas do cliente via MCP.'],
                ['🔷 Azure DevOps', 'Importa work items e histórico de TR do projeto configurado.'],
              ].map(([title, desc]) => (
                <li key={title} className="flex gap-2 p-3 rounded-lg border border-border">
                  <div>
            ...
            <h3 className="font-semibold text-sm mb-2 mt-4">Re-sincronizar uma seção individualmente</h3>
            <p className="text-sm text-muted-foreground">Na tela de edição de qualquer seção automática, o botão <strong>Re-sincronizar</strong> aparece no canto superior direito — atualiza só aquela seção sem rodar todas as integrações.</p>
          </SectionBlock>

          <SectionBlock id="status" title="4. Fluxo de Status do Relatório">
            <p className="text-sm text-muted-foreground mb-4">O relatório passa por quatro status ao longo do seu ciclo de vida:</p>
            <Steps items={[
              { title: 'Rascunho — Estado inicial', body: 'Líderes de Tribo e Projetos e Produtos podem editar livremente todas as seções e sincronizar dados.' },
              { title: 'Em Revisão — Enviado para revisão', body: 'Líderes ainda podem editar. Projetos e Produtos passa a somente leitura. C-Level e Superadmin podem aprovar ou devolver para Rascunho.' },
              { title: 'Aprovado — Ninguém pode editar', body: 'Apenas C-Level ou Superadmin podem publicar ou reabrir o relatório.' },
          ...
          <SectionBlock id="pptx" title="5. Gerar o PPTX">
            <p className="text-sm text-muted-foreground mb-4">Quando o relatório estiver pronto, clique em <strong>⬇ Gerar PPTX</strong> no cabeçalho. O download inicia automaticamente.</p>
            <Steps items={[
              { title: 'Revise os indicadores', body: 'Certifique-se de que as seções que devem entrar estão como ✅ ou 🟡. Seções com "Ocultar slide" ativo não entram no arquivo.' },
              { title: 'Clique em Gerar PPTX', body: 'O botão mostra "Gerando..." enquanto o arquivo é montado. O processo leva alguns segundos.' },
              { title: 'Download automático', body: 'O arquivo é baixado com o nome do contrato e mês/ano. Abra no PowerPoint ou Google Slides para revisar antes de enviar ao cliente.' },
            ]} />
            <Callout type="tip">O PPTX pode ser gerado em qualquer status — não é necessário que o relatório esteja Publicado para exportar.</Callout>
          </SectionBlock>
          ...
          <SectionBlock id="semaforo" title="Semáforo de Integrações">
            <p className="text-sm text-muted-foreground mb-4">Na listagem de relatórios, cada contrato exibe quatro pontos coloridos indicando o status de configuração de cada integração. A ordem é: <strong>Asana · Fireflies · Milvus · Azure DevOps</strong>.</p>
              ...
              {[
                ...
                <div key={i} className="flex items-center gap-3">
                  <div className="flex gap-1.5">
                    {row.dots.map((c, j) => <div key={j} className={`w-3 h-3 rounded-full ${c}`} />)}
                  </div>
                  <span className="text-sm text-muted-foreground">{row.label}</span>
                </div>
              ))}
            </div>
            <Callout type="warn">Um ponto cinza indica que aquela integração não está configurada. Solicite ao administrador que configure os IDs e chaves na tela ⚙️ do contrato.</Callout>
          </SectionBlock>

          <SectionBlock id="duvidas" title="Dúvidas Frequentes">
            {[
              { q: 'O relatório foi criado mas as seções automáticas estão vazias', a: 'A sincronização acontece em segundo plano após a criação. Aguarde alguns segundos e clique em 🔄 Sincronizar Dados para forçar a atualização.' },
              { q: 'Tentei sincronizar mas uma integração deu erro', a: 'Verifique o painel de resultados que aparece após o sync. O erro mostra o motivo exato. Repasse a mensagem ao administrador.' },
              { q: 'Não consigo mais editar o relatório', a: 'O relatório está em status Aprovado ou Publicado. Um aviso aparece na tela. Solicite ao C-Level ou Superadmin que altere o status se precisar fazer correções.' },
              { q: 'Quero aproveitar o conteúdo do mês anterior', a: 'Ao criar um novo relatório, o sistema oferece a opção de cópia. Se o relatório já foi criado, use o botão Copiar mês anterior no cabeçalho.' },
              { q: 'Criei o relatório no mês errado', a: 'Não é possível alterar o mês após a criação. Delete o relatório incorreto (somente em status Rascunho) e crie um novo para o mês correto.' },
              { q: 'O PPTX gerou sem alguma seção', a: 'Verifique se o toggle "Ocultar slide" está ativado naquela seção. Desative-o e gere o arquivo novamente.' },
            ].map((item, i) => (
