...
export async function generatePptx(input: GeneratePptxInput): Promise<void> {
  ...
  // ── SLIDE 2: SUMÁRIO ───────────────────────────────────────────────────────
  {
    ...
    sumItems.forEach((item, i) => {
      s.addText(`${i + 1}.  ${item}`, { x: 1.5, y: 0.82 + i * 0.36, w: 7, h: 0.34, fontSize: 13, color: AZUL_MEDIO });
    });
  }
  ...
  // ── SLIDE 3: OBJETIVO ─────────────────────────────────────────────────────
  {
    ...
    s.addText((obj.texto as string) ?? "Apresentar as principais entregas, indicadores e oportunidades identificadas no período de referência, evidenciando a evolução do contrato e o engajamento das partes envolvidas.", { x: 0.5, y: 1.0, w: 9, h: 1.2, fontSize: 13, color: CINZA_TEXTO, wrap: true });
  ...
  // ── SLIDE 4: PAINEL EXECUTIVO ─────────────────────────────────────────────
  {
    ...
    cards.forEach((c, i) => {
      ...
      const status = normalizeStatus((painel[c.key] as string) ?? "adequado");
  ...
  // ── SLIDE 5: HISTÓRICO TR (condicional) ───────────────────────────────────
  {
    const htr = sections["historico_tr"] ?? {};
    ...
    if (linhasHtr.length > 0) {
      ...
      s.addTable(tableData as any, { x: 0.4, y: 1.15, w: 9.2, fontSize: 10, border: { pt: 0.5, color: "D0DCE8" }, rowH: 0.42, colW: [7.5, 1.7], align: "left", valign: "middle" });
    }
  }

  // ── SLIDE 6: EVOLUÇÃO E INOVAÇÃO (condicional) ────────────────────────────
  {
    const evo = sections["evolucao_inovacao"] ?? {};
    const tags = (evo.contagem_por_tag ?? evo.tags) as Record<string, number> | undefined;
    const pctEvo = Number(evo.percentual_inovacao ?? evo.percentualInovacao ?? 0);
    ...
    if (tags || pctEvo > 0) {
      ...
      s.addText(mesAno, { x: 0.5, y: 0.72, w: 9, h: 0.28, fontSize: 11, bold: true, color: "555555" });
      if (tags) {
        Object.entries(tags).forEach(([tag, count], i) => {
          kpiCard(s, 0.4 + i * 2.3, 1.05, 2.0, 1.1, tag, String(count), AZUL_MEDIO);
        });
      }
      s.addText(`Total de entregas: ${totalEvo}`, { x: 0.5, y: 2.35, w: 4, h: 0.3, fontSize: 11, color: CINZA_TEXTO });
      s.addText(`% Inovação: ${pctEvo}%`, { x: 0.5, y: 2.7, w: 4, h: 0.3, fontSize: 13, bold: true, color: AZUL_ESCURO });
      statusBadge(s, 4.8, 2.6, 2.4, (evo.status as string) ?? "adequado");
      if (evo.analise) {
  ...
  // ── SLIDE 8: TAREFAS PRIORIZADAS (condicional) ────────────────────────────
  {
    const prio = sections["priorizadas"] ?? {};
  ...
  // ── SLIDE 9: DEMONSTRATIVO DE HORAS (condicional) ─────────────────────────
  {
    const demo = sections["demonstrativo_horas"] ?? {};
    ...
    if (linhasDemo.length > 0) {
      ...
      s.addTable(tableData as any, { x: 0.4, y: 1.1, w: 9.2, fontSize: 10, border: { pt: 0.5, color: "D0DCE8" }, rowH: 0.42, colW: [2.8, 2.0, 1.5, 1.3, 1.6], align: "left", valign: "middle" });
      if (demo.legenda) {
  ...
  // ── SLIDE 10: EFICIÊNCIA OPERACIONAL ──────────────────────────────────────
  {
    ...
    statusBadge(s, 0.5, 1.1, 2.8, (efOp.status as string) ?? "adequado");
  ...
  // ── SLIDE 11: EFICIÊNCIA E PREVISIBILIDADE (condicional) ──────────────────
  {
    const efPrev = sections["eficiencia_previsibilidade"] ?? {};
    ...
    if (temDados) {
      ...
      s.addText(mesAno, { x: 0.5, y: 0.72, w: 9, h: 0.28, fontSize: 11, bold: true, color: "555555" });
      statusBadge(s, 0.5, 1.1, 2.8, (efPrev.status as string) ?? "adequado");
      const kpis = [
  ...
  // ── SLIDE 12: DESEMPENHO DA APLICAÇÃO (condicional) ───────────────────────
  {
    const desemp = sections["desempenho_aplicacao"] ?? {};
  ...
  // ── SLIDE 13: ENGAJAMENTO DO USUÁRIO (condicional) ────────────────────────
  {
    const eng = sections["engajamento_usuario"] ?? {};
  ...
  // ── SLIDE 14: MATURIDADE DA PLATAFORMA (condicional) ──────────────────────
  {
    const mat = sections["maturidade_plataforma"] ?? {};
  ...
  // ── SLIDE 15: TREINAMENTOS / REUNIÕES ─────────────────────────────────────
  {
    ...
    // Fireflies salva em content.linhas; legado usava content.reunioes
