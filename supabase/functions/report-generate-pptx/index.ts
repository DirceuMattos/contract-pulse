...
serve(async (req) => {
  ...
  try {
    ...
    const { data: report } = await supabase.from("monthly_reports").select("*, contracts(*, clients(*))").eq("id", reportId).single();
    ...
    function headerBar(slide: any, titulo: string) {
      ...
      slide.addText("bnp", { x: 8.5, y: 0.1, w: 1.3, h: 0.45, fontSize: 18, bold: true, color: BRANCO, align: "right", valign: "middle" });
    }
    ...
    function statusBadge(slide: any, x: number, y: number, w: number, status: string) {
      ...
      slide.addShape("roundRect", { x, y, w, h: 0.34, fill: { color: sc.cor }, line: { color: sc.cor }, rectRadius: 0.05 });
      slide.addText(sc.label, { x, y, w, h: 0.34, fontSize: 11, bold: true, color: BRANCO, align: "center", valign: "middle", margin: 0 });
    }

    function kpiCard(slide: any, x: number, y: number, w: number, h: number, label: string, valor: string, cor?: string) {
      slide.addShape("roundRect", { x, y, w, h, fill: { color: CINZA_CLARO }, shadow: { type: "outer", color: "000000", blur: 4, offset: 1, angle: 45, opacity: 0.1 }, rectRadius: 0.08, line: { color: "E0E7EF", width: 0.5 } });
    ...
    // ── SLIDE 1: CAPA ────────────────────────────────────────────────────────
    {
      ...
      s.addText("Relatório Mensal de Atividades", { x: 0.4, y: 2.0, w: 5.5, h: 0.8, fontSize: 26, bold: true, color: AZUL_ESCURO });
      const capa = sectionMap["capa"] ?? {};
      s.addText(mesAno, { x: 0.4, y: 3.1, w: 5.5, h: 0.35, fontSize: 14, bold: true, color: CINZA_TEXTO });
      s.addText(`Projeto: ${(capa.projeto as string) || nomeContrato}`, { x: 0.4, y: 3.5, w: 5.5, h: 0.3, fontSize: 11, color: CINZA_TEXTO });
      s.addText((capa.cliente as string) || nomeCliente, { x: 0.4, y: 3.82, w: 5.5, h: 0.3, fontSize: 11, color: CINZA_TEXTO });
      s.addText(`Contrato: ${(capa.numeroContrato as string) || numeroContrato}`, { x: 0.4, y: 4.1, w: 5.5, h: 0.3, fontSize: 11, bold: true, color: AZUL_ESCURO });
    }
    ...
    // ── SLIDE 2: SUMÁRIO ─────────────────────────────────────────────────────
    {
      ...
      headerBar(s, "Sumário");
      const sumItems = [
        "Objetivo do relatório","Painel executivo","Histórico evolutivo do Termo de Referência",
        "Evolução e Inovação","Evolução e Inovação / Entregas","Tarefas Priorizadas",
        "Demonstrativo de Horas","Eficiência Operacional","Eficiência e Previsibilidade",
        "Engajamento e Experiência do Usuário","Maturidade e Gestão da Plataforma",
        "Treinamentos / Reuniões","Oportunidades e Fatores de Atenção",
      ];
    ...
    // ── SLIDE 3: OBJETIVO ────────────────────────────────────────────────────
    {
      ...
      const texto = (obj.texto as string) ?? "Apresentar as principais entregas, indicadores e oportunidades identificadas no período de referência.";
    ...
    // ── SLIDE 4: PAINEL EXECUTIVO ────────────────────────────────────────────
    {
      ...
      cards.forEach((c, i) => {
        ...
        const status = ((painel[c.key] as string) ?? "adequado").toLowerCase().replace(" ", "").replace("alta performance", "alta").replace("atenção", "atencao").replace("crítico", "critico");
      ...
      if (painel.observacoes) {
        s.addText(painel.observacoes as string, { x: 0.4, y: 4.65, w: 9.2, h: 0.4, fontSize: 9, color: "888888", italic: true });
      }
    }
    ...
    // ── SLIDE 5: HISTÓRICO TR ────────────────────────────────────────────────
    {
      ...
      if (linhasHtr.length > 0) {
        ...
        s.addShape("roundRect", { x: 7.5, y: 0.72, w: 2.0, h: 0.32, fill: { color: AZUL_ESCURO }, line: { color: AZUL_ESCURO }, rectRadius: 0.05 });
        s.addText(`${pct}% concluído`, { x: 7.5, y: 0.72, w: 2.0, h: 0.32, fontSize: 10, bold: true, color: BRANCO, align: "center", valign: "middle" });
        const tableData = [
        ...
        s.addTable(tableData, { x: 0.4, y: 1.15, w: 9.2, fontSize: 10, border: { pt: 0.5, color: "D0DCE8" }, rowH: 0.42, colW: [7.5, 1.7], align: "left", valign: "middle" });
      }
    }
    ...
    // ── SLIDE 6: EVOLUÇÃO E INOVAÇÃO ─────────────────────────────────────────
    {
      ...
      if (tags || evo.percentual_inovacao !== undefined) {
        ...
        s.addText(`Total de entregas: ${totalEvo}`, { x: 0.5, y: 2.35, w: 4, h: 0.3, fontSize: 11, color: CINZA_TEXTO });
        s.addText(`% Inovação: ${pct}%`, { x: 0.5, y: 2.7, w: 4, h: 0.3, fontSize: 13, bold: true, color: AZUL_ESCURO });
        const statusEvo = (evo.status as string) ?? "adequado";
        statusBadge(s, 4.8, 2.6, 2.4, statusEvo);
        if (evo.analise) {
          s.addText(evo.analise as string, { x: 0.5, y: 3.2, w: 9, h: 1.5, fontSize: 10, color: CINZA_TEXTO, wrap: true });
        }
      }
    }
    ...
    // ── SLIDE 7: ENTREGAS ────────────────────────────────────────────────────
    {
      ...
      const tarefas = ((entregas.tarefas ?? entregas.linhas) as Array<{ nome?: string; tarefa?: string; status: string; categoria: string }>) ?? [];
      if (tarefas.length > 0) {
        const tableData = [
        ...
        s.addTable(tableData, { x: 0.4, y: 1.55, w: 9.2, fontSize: 10, border: { pt: 0.5, color: "D0DCE8" }, rowH: 0.42, colW: [5.0, 1.4, 2.8], align: "left", valign: "middle" });
    ...
    // ── SLIDE 8: TAREFAS PRIORIZADAS ─────────────────────────────────────────
    {
      ...
      const tarefasPrio = ((prio.tarefas ?? prio.linhas) as Array<{ nome?: string; tarefa?: string; status: string; categoria: string }>) ?? [];
      if (tarefasPrio.length > 0) {
        ...
        if (prio.total_backlog) {
          s.addText(`Backlog: ${prio.total_backlog} itens`, { x: 0.4, y: 4.75, w: 4, h: 0.3, fontSize: 10, color: "888888", italic: true });
        }
      }
    }
    ...
    // ── SLIDE 9: DEMONSTRATIVO DE HORAS ──────────────────────────────────────
    {
      ...
      const linhasDemo = (demo.linhas as Array<{ recurso: string; funcao?: string; dedicacao?: string; unidade: string; quantidade: number }>) ?? [];
      if (linhasDemo.length > 0) {
        ...
        s.addTable(tableData, { x: 0.4, y: 1.1, w: 9.2, fontSize: 10, border: { pt: 0.5, color: "D0DCE8" }, rowH: 0.42, colW: [2.8, 2.0, 1.5, 1.3, 1.6], align: "left", valign: "middle" });
        if (demo.legenda) {
          s.addText(demo.legenda as string, { x: 0.4, y: 4.7, w: 9.2, h: 0.35, fontSize: 9, color: "888888", italic: true });
        }
      }
    }
    ...
    // ── SLIDE 10: EFICIÊNCIA OPERACIONAL ─────────────────────────────────────
    {
      ...
      s.addText((efOp.analise as string) || "Análise a ser preenchida.", { x: 3.75, y: 1.6, w: 5.6, h: 3.2, fontSize: 10, color: CINZA_TEXTO, valign: "top" });
    }

    // ── SLIDE 11: EFICIÊNCIA E PREVISIBILIDADE ────────────────────────────────
    {
      const efPrev = sectionMap["eficiencia_previsibilidade"] ?? {};
      const temDados = efPrev.frequencia_deploy || efPrev.frequenciaDeploy || efPrev.lead_time || efPrev.leadTime || efPrev.demandas;
      if (temDados) {
        const s = pres.addSlide();
        s.background = { color: BRANCO };
        ...
        if (efPrev.analise) {
          ...
          s.addText(efPrev.analise as string, { x: 5.55, y: 1.6, w: 3.8, h: 3.2, fontSize: 10, color: CINZA_TEXTO, valign: "top" });
        }
      }
    }
    ...
    // ── SLIDE 12: DESEMPENHO DA APLICAÇÃO ────────────────────────────────────
    {
      ...
      if (desemp.status || desemp.analise) {
        ...
        if (desemp.analise) {
          s.addText(desemp.analise as string, { x: 0.5, y: 1.65, w: 9, h: 3.0, fontSize: 11, color: CINZA_TEXTO, wrap: true });
        }
      }
    }
    ...
    // ── SLIDE 13: ENGAJAMENTO DO USUÁRIO ─────────────────────────────────────
    {
      ...
      if (temEngDados) {
        ...
        if (eng.analise) {
          s.addText(eng.analise as string, { x: 0.5, y: 4.1, w: 9, h: 0.8, fontSize: 10, color: CINZA_TEXTO, wrap: true });
        }
      }
    }
    ...
    // ── SLIDE 14: MATURIDADE DA PLATAFORMA ───────────────────────────────────
    {
      ...
      if (metricas.length > 0 || mat.analise) {
        ...
        if (mat.analise) {
          ...
          s.addText(mat.analise as string, { x: 0.5, y: yBase, w: 9, h: 1.5, fontSize: 10, color: CINZA_TEXTO, wrap: true });
        }
      }
    }
    ...
    // ── SLIDE 15: TREINAMENTOS / REUNIÕES ────────────────────────────────────
    {
      ...
      const reunioes = ((trein.linhas ?? trein.reunioes) as Array<{ tipo: string; data: string; horario?: string; descricao: string }>) ?? [];
      if (reunioes.length > 0) {
        ...
        s.addTable(tableData, { x: 0.4, y: 1.1, w: 9.2, fontSize: 10, border: { pt: 0.5, color: "D0DCE8" }, rowH: 0.55, colW: [2.0, 1.5, 5.7], align: "left", valign: "middle" });
      } else {
        emptyMsg(s, "Nenhuma reunião registrada para o período.");
      }
      s.addText((trein.rodape as string) ?? "Além das reuniões e treinamentos realizados, a equipe da BNP presta apoio consultivo contínuo aos gestores.", { x: 0.4, y: 4.7, w: 9.2, h: 0.5, fontSize: 10, bold: true, color: CINZA_TEXTO, italic: true });
    }
    ...
    // ── SLIDE 16: OPORTUNIDADES E FATORES DE ATENÇÃO ─────────────────────────
    {
      ...
      if (itens.length > 0) {
        ...
        s.addTable(tableData, { x: 0.4, y: 1.1, w: 9.2, fontSize: 11, border: { pt: 0.5, color: "D0DCE8" }, rowH: 0.55, colW: [7.0, 2.2], align: "left", valign: "middle" });
      } else {
    ...
    return new Response(JSON.stringify({ fileBase64: base64, filename }), { headers: { "Content-Type": "application/json", "Access-Control-Allow-Origin": "*" } });

  } catch (error) {
    return new Response(JSON.stringify({ error: (error as Error).message }), { status: 500, headers: { "Content-Type": "application/json", "Access-Control-Allow-Origin": "*" } });
  }
});
